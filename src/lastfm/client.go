...
func (c *LastFmClient) GetLibraryPlaylist(username string, page int) (PlaylistWrapperRs, error) {
	return c.getStationPlaylist(username, "library", page)
}

func (c *LastFmClient) GetMixPlaylist(username string, page int) (PlaylistWrapperRs, error) {
	return c.getStationPlaylist(username, "mix", page)
}

func (c *LastFmClient) GetRecommendedPlaylist(username string, page int) (PlaylistWrapperRs, error) {
	return c.getStationPlaylist(username, "recommended", page)
}

func (c *LastFmClient) getStationPlaylist(username string, station string, page int) (PlaylistWrapperRs, error) {
	req, err := http.NewRequest(http.MethodGet, fmt.Sprintf("https://www.last.fm/player/station/user/%s/%s", username, station), nil)
	if err != nil {
		return PlaylistWrapperRs{}, err
	}
