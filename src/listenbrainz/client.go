...
func (c *ListenBrainzClient) ValidateToken(token string) (*ValidateTokenResponse, error) {
...
func (c *ListenBrainzClient) SubmitListens(token string, request SubmitListensRequest) error {
...
func (c *ListenBrainzClient) GetPlaylistsCreatedFor(token string, username string, count int, offset int) (PlaylistsResponse, error) {
	slog.Log(
		context.Background(),
		config.LevelTrace,
...
func (c *ListenBrainzClient) GetPlaylist(token string, id string) (PlaylistResponse, error) {
	slog.Log(
		context.Background(),
		config.LevelTrace,
	...
	if res.StatusCode == http.StatusNotFound {
		return PlaylistResponse{}, fmt.Errorf("playlist with id `%s` was not found", id)
	} else if res.StatusCode != http.StatusOK {
		return PlaylistResponse{}, fmt.Errorf("http code %d when requesting %s", res.StatusCode, req.URL)
	}
