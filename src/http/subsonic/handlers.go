...
func GetHandlers(appCtx *appcontext.Context) map[string]http.HandlerFunc {
	rawHandlers := map[string]http.HandlerFunc{
		...
		"/getPlaylists":             util.AsHandlerFunc(handlers.NewGetPlaylistsHandler(appCtx.PlaylistStorage).Handle),
		"/getPlaylist":              util.AsHandlerFunc(handlers.NewGetPlaylistHandler(appCtx.PlaylistStorage).Handle),
